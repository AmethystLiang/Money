...
class StockController:
    ...
    def buyStock(self):
        ...
        date_collapsed = datetime.timedelta(days=self.env.now*7)
        ...
        stock_price = stock.get_historical_prices(current_date,current_date)
        print "The current price of the stock is %d dolars per share. " %stock_price
        ...
        if wanna_buy:
            ...
            amount = check_positive_valid_input("Enter your number below:")
            cost = amount*stock_price
            print "The cost for buying %d shares of %s stock is %d dollars" %(amount,stock_name,cost)
            if self.player.buy_property(cost,"Sorry, you don't have enough money for buying the stock."):
                self.STOCKS[stock_name].purchased = 1
        else :
            return
